 ...
 * QuartzSessionValidationScheduler 用以替换shiro-quartz文件用来支持quartz.x系列版本 fbf
 * 20160225打包 基于Quartz 2.* 版本的实现
	...
	public void enableSessionValidation() {
		...
		try {
					...
					.withSchedule(
							SimpleScheduleBuilder.simpleSchedule()
									.withIntervalInMilliseconds(
											sessionValidationInterval)).build();// <span
																				// style="color:#ff0000;">Quartz
																				// 2中的实现</span>
			...
			detail.getJobDataMap()
					.put(SESSION_MANAGER_KEY, this.sessionManager);
